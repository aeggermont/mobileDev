...
import tv.middleware as tv_middleware
...
class CompressorNode(template.Node):
    ...
    def render_offline(self, context, forced):
        ...
        if (settings.COMPRESS_ENABLED and
                settings.COMPRESS_OFFLINE) and not forced and not self.nocompress:
    ...
    def render_cached(self, compressor, forced):
        ...
        """
        if settings.COMPRESS_ENABLED and not forced and not self.nocompress:
    ...
    def render(self, context, forced=False):
        ...
        if self not in context.render_context:
            context.render_context[self] = self.render_cached(compressor, forced)
        cache_key, cache_content = context.render_context[self]
